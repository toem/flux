...
void example11() {
	...
	// output file
	FILE *out = fopen("traces/example11.recTr", "wb");
	...
	// buffers
	flxBuffer buffer2 = flxCreateSimpleBuffer(memoryBuffer + bufferSize * 1, bufferSize, flxWriteToFile, out); // writes to files
	flxBuffer buffer1 = flxCreateSimpleBuffer(memoryBuffer + bufferSize * 0, bufferSize, flxCompressLz4, buffer2); // compresses to buffer 2
	...
	if (trace != 0) {
		...
		flxWriteArrayDef(trace, 42, 0, "x", 0);
		flxWriteArrayDef(trace, 42, 1, "y", 0);
		flxWriteArrayDef(trace, 43, 0, "x", 0);
		flxWriteArrayDef(trace, 43, 1, "y", 0);
		flxWriteArrayDef(trace, 44, 0, "state", 0);
		flxWriteArrayDef(trace, 44, 1, "done", 0);
		...
		for (int n = 0; n < 50000; n++) {
			...
			flxWriteIntAt(trace, 2, 0, current , 0, &i1, sizeof(int), 0);
			...
			if (n == 100000) {
				flxWriteNoneAt(trace, 12, 0, current + 7, 0);
				flxWriteNoneAt(trace, 13, 0, 0, 1);
			} else if (n < 100000 || n >= 200000) {
				flxWriteFloatAt(trace, 12, 0, current + 7, 0, &f1, sizeof(float));
				flxWriteFloatAt(trace, 13, 0, 0, 1, &f2, sizeof(double));
			}

			// logic
			flxWriteLogicTextAt(trace, 22, 0, current + 8, 0, 0, "1", 1, 1);
			flxWriteLogicTextAt(trace, 23, 0, 0, 1, 0, "0011x1", 6, 6);
			flxWriteLogicTextAt(trace, 24, 0, 0, 1, 0, "u", 1, 1);
			flxWriteLogicTextAt(trace, 25, 0, 0, 1, 0, "x", 1, 1);
			...
			ia[0] = i1;
			ia[1] = i2;
			flxWriteIntArrayAt(trace, 42, 0, 0, 1, ia, sizeof(int), 0, 2, 0);

			// float array
			fa[0] = f1;
			fa[1] = f2;
			flxWriteFloatArrayAt(trace, 43, 0, 0, 1, fa, sizeof(float), 2, 0);

			// ecent array
			ea[0] = n % 2;
			ea[1] = (n + 1) % 2;
			flxWriteEventArrayAt(trace, 44, 0, 0, 1, ea, 2, 0);
		}
